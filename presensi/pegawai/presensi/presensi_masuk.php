...
<body class="bg-gray-100 min-h-screen p-6">
  ...
  <div class="flex justify-between items-center mb-4">
        <h2 class="text-2xl font-bold">Presensi Masuk</h2>
        <a href="../home/index.php" class="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700">Keluar</a>
  </div>
    ...
    <div class="flex flex-col lg:flex-row gap-6">
      ...
      <div class="lg:w-1/2">
        ...
        <div class="flex gap-3">
          ...
          <button id="btnSubmit" class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700">Masuk</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <!-- Hidden Form -->
  <form id="formPresensiMasuk" style="display:none;">
  ...
  <script>
    ...
    document.getElementById('btnSubmit').addEventListener('click', function () {
      const form = document.getElementById('formPresensiMasuk');
      const data = new FormData(form);

      fetch('proses_presensi_masuk.php', {
        method: 'POST',
        body: data
      })
        .then(res => res.json())
